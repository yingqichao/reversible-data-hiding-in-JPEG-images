...
public class jpeglibrary {
	...
	private String waterBinary = new String();
	...
	public void operation(){
		...
		try{
			file = readStream(im);
//			water = readStream(w);
			String wmStr = Util.readWatermark(w);
			water = Util.StreamFromString(wmStr);
		}
		catch (Exception e){
...
//		return res;
	...
	public int[][] cutX(byte[] x){
		//�ҵ����йؼ��ֶ�λ�ã�3�д洢��1�б�־�����У�������ֹ��
		//���⣬x�е����ݶ���Ҫ��unsignDecoderתΪ0-255
	...
	public void cutdata(byte[] x,int[][] d){
		...
		while(i<im.length){
			...
			if((i!=im.length-1)&&(Tool.unsignDecoder(im[i])==255)){
				...
				case 255:i++;//�ޱ��棬������255
				case 208:im2[j]=im[i];j++;i++;RSTnum++;break;//�������RST�ֶ�
				...
				default:im2[j]=im[i+1];j++;i+=2;break;//����255���������ķ�0/255
				}
			}
			else{
				im2[j]=im[i];j++;i++;
